 ...
 * @version 2020-09-30
 */
@Table(name="tab_aqd_qingqiu_log", alias="a", columns={
		@Column(name="rowid", attrName="rowid", label="编号", isPK=true),
		@Column(name="imei", attrName="imei", label="设备IMEI号", queryType=QueryType.LIKE),
		@Column(name="name", attrName="name", label="工人姓名", queryType=QueryType.LIKE),
		@Column(name="warning_type", attrName="warningType", label="报警请求类型"),
		@Column(name="status1", attrName="status1", label="处理状态"),
		@Column(name="createtime", attrName="createtime", label="创建时间"),
		@Column(name="logincode", attrName="logincode", label="logincode", isQuery=false),
	}, orderBy="a.rowid DESC"
)
public class TabAqdQingqiuLog extends DataEntity<TabAqdQingqiuLog> {
	
	private static final long serialVersionUID = 1L;
	private String rowid;		// 编号
	private String imei;		// 设备IMEI号
	private String name;		// 工人姓名
	private String warningType;		// 报警请求类型
	private Integer status1;		// 处理状态
	private Date createtime;		// 创建时间
	private String logincode;		// logincode
	
	public TabAqdQingqiuLog() {
		this(null);
	}

	public TabAqdQingqiuLog(String id){
		super(id);
	}
	...
	@Length(min=0, max=200, message="报警请求类型长度不能超过 200 个字符")
